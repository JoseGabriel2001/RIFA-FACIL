...
async def create_raffle(raffle_data: RaffleCreate, user: dict = Depends(get_current_user)):
    ...
    raffle_doc = {
        ...
        "tickets": create_tickets(
            raffle_data.total_tickets, 
            raffle_data.excluded_numbers or []
        ),
...
async def list_raffles(user: dict = Depends(get_current_user)):
    ...
    for raffle in raffles:
        ...
        raffle['reserved_count'] = len([t for t in tickets if t['status'] == 'reserved'])
        raffle['available_count'] = len([t for t in tickets if t['status'] == 'available'])
...
async def preselect_winner(
    ...
    ticket = next(
        (t for t in raffle['tickets'] if t['number'] == request.winning_number),
        None
    )
    ...
    if not ticket:
        raise HTTPException(
            status_code=400, 
            detail="Número de boleto no válido"
        )
    
    if ticket['status'] != 'sold':
        raise HTTPException(
            status_code=400, 
            detail="Solo puedes preseleccionar un boleto vendido"
        )
    
    # Save preselection
    await db.raffles.update_one(
        {"id": raffle_id},
        {"$set": {"preselected_winner": request.winning_number}}
    )
...
async def spin_wheel(raffle_id: str, user: dict = Depends(get_current_user)):
    ...
    if len(sold_tickets) == 0:
        raise HTTPException(
            status_code=400, 
            detail="No hay boletos vendidos para sortear"
        )
    ...
    await db.raffles.update_one(
        {"id": raffle_id},
        {"$set": {"current_spin_count": current_spin}}
    )
    ...
    if is_final_spin:
        ...
        winner_ticket = next(
            (t for t in raffle['tickets'] if t['number'] == winning_number),
            None
        )
...
async def set_winner(
    ...
    if not ticket or ticket['status'] != 'sold':
        raise HTTPException(
            status_code=400, 
            detail="Número de boleto no válido o no vendido"
        )
...
async def assign_tickets(
    ...
    for num in request.ticket_numbers:
        ticket = next(
            (t for t in raffle['tickets'] if t['number'] == num),
            None
        )
        if not ticket:
            raise HTTPException(
                status_code=400, 
                detail=f"Boleto #{num} no existe"
            )
        if ticket['status'] != 'available':
            raise HTTPException(
                status_code=400, 
                detail=f"Boleto #{num} no está disponible"
            )
...
async def validate_order(
    ...
    order = await db.cash_orders.find_one(
        {"id": request.order_id, "raffle_id": raffle_id},
        {"_id": 0}
    )
    ...
    if order['status'] != 'pending':
        raise HTTPException(
            status_code=400, 
            detail="Esta orden ya fue procesada"
        )
