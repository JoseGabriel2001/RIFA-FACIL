...
BASE_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://spin-raffle.preview.emergentagent.com').rstrip('/')
API = f"{BASE_URL}/api"
...
class TestUserLogin:
    ...
    def test_auth_me_with_valid_token(self):
        ...
        assert response.status_code == 200, f"Auth me should succeed, got {response.status_code}"
...
class TestFreePlanLimits:
    ...
    def setup_test_user(self):
        ...
        raffles_resp = requests.get(f"{API}/raffles", headers={
            "Authorization": f"Bearer {self.user_token}"
        })
        if raffles_resp.status_code == 200:
            for raffle in raffles_resp.json():
                requests.delete(f"{API}/raffles/{raffle['id']}", headers={
                    "Authorization": f"Bearer {self.user_token}"
                })
    
    def _create_raffle(self, title_suffix):
        """Helper to create a raffle"""
        return requests.post(f"{API}/raffles", json={
    ...
    def test_create_first_raffle_success(self):
        ...
        assert response.status_code == 200, f"First raffle should succeed, got {response.status_code}: {response.text}"
    ...
    def test_create_second_raffle_success(self):
        ...
        assert resp2.status_code == 200, f"Second raffle should succeed, got {resp2.status_code}: {resp2.text}"
    ...
    def test_create_third_raffle_fails_free_plan(self):
        ...
        print(f"Third raffle detail: {resp3.json() if resp3.status_code < 500 else resp3.text}")
        
        assert resp3.status_code == 403, f"Third raffle should fail with 403, got {resp3.status_code}"
        assert "limitado" in resp3.json().get("detail", "").lower() or "premium" in resp3.json().get("detail", "").lower()
...
class TestMercadoPagoEndpoint:
    ...
    def setup_test_data(self):
        ...
        raffle_resp = requests.post(f"{API}/raffles", json={
    ...
    def test_mercadopago_create_preference_returns_503(self):
        ...
        response = requests.post(f"{API}/payments/mercadopago/create-preference", json={
        ...
        # Should return 503 because MERCADOPAGO_ACCESS_TOKEN is not configured
        ...
        if response.status_code == 503:
            ...
            result = subprocess.run([
                ...
                '-d', '{"raffle_id": "test", "ticket_numbers": [1], "buyer_name": "Test", "buyer_email": "test@test.com", "origin_url": "https://test.com"}'
            ], capture_output=True, text=True)
            local_status = result.stdout.strip()
            print(f"Local test status: {local_status}")
            assert local_status == "503", f"Local endpoint should return 503, got {local_status}"
        else:
    ...
    def test_mercadopago_status_endpoint_404(self):
        ...
        response = requests.get(f"{API}/payments/mercadopago/status/nonexistent-transaction-id")
...
class TestGoogleAuthSession:
    ...
    def test_google_session_without_session_id_fails(self):
        ...
        assert response.status_code == 400, f"Expected 400 without session_id, got {response.status_code}"
...
class TestPublicRaffleEndpoint:
    ...
    def test_public_raffle_endpoint_returns_data(self):
        ...
        assert response.status_code == 200, f"Public raffle should return 200, got {response.status_code}"
